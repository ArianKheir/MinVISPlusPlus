    ...
    def __call__(self, dataset_dict, paste_dataset_dict=None):
        ...
        for frame_idx in selected_idx:
            ...
            if instances.has("gt_masks"):
                ...
                instances.gt_masks = BitMasks(torch.empty((0, *image_shape)))
                #Adding the bbox centers to the DataSetMapper(Adding zeros cuase it's none)
                instances.set("gt_centers", torch.zeros((0, 2), dtype=torch.float32))
...
class CocoClipDatasetMapper:
    ...
    def make_annos_to_instances(self, original_image, img_annos, augmentations):
        ...
        if hasattr(instances, 'gt_masks'):
            ...
            instances.gt_masks = BitMasks(gt_masks.bool())
            instances = filter_empty_instances(instances)
            if(len(instances) > 0):
                gt_boxes = instances.gt_masks.get_bounding_boxes()
                centers = gt_boxes.get_centers().to(torch.float32)
                normalized_centers = centers / torch.tensor([w, h], dtype=torch.float32, device=centers.device)
                instances.set("gt_centers", normalized_centers)
                instances = filter_empty_instances(instances)
            else:
            ...
            instances.gt_masks = BitMasks(torch.zeros((0, h, w), dtype=torch.bool))
            instances.set("gt_centers", torch.zeros((0, 2), dtype=torch.float32))
            instances = filter_empty_instances(instances)
